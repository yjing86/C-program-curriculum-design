...
void Weibo::creat() {
	Limit_JY a;
	weibo_friendmanage.read_friendlist();
	weibo_groupmanage.read_grouplist();

	FILE* open_file = fopen("./Weibo/source/weibo_id_max.txt", "r");
	long temp = 0;
	fscanf(open_file, "%ld", &temp);
	id = ++temp;
	cout << "您的帐号ID为" << id << endl;
	fclose(open_file);
	FILE* close_file = fopen("./Weibo/source/weibo_id_max.txt", "w");
	...
	char filename[30];
	sprintf(filename, "./Weibo/person/weibo_%ld.txt", id);
	...
	weibo_friendmanage.save_friendlist();
	weibo_groupmanage.save_grouplist();
}
void Weibo::show_all()
{
	show();
...
Weibo::Weibo(long qid) {
	...
	while (panduan) {
		panduan = 0;
		sprintf(filename, "./Weibo/person/weibo_%ld.txt", qid);
		fp = fopen(filename, "r");
		if (fp == NULL) {
